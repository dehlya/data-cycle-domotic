...
def run():
    ...
    print(f"  Creating gold schema and tables...")

    # Step 1: Create schema with admin (if available)
    if DB_ADMIN_URL:
        ...
        print("  Warning: DB_ADMIN_URL not set, assuming schema exists")

    # Step 2: Create tables with app user (so app user owns them)
    # Remove the CREATE SCHEMA line from DDL since we already did it
    tables_ddl = GOLD_DDL.replace("CREATE SCHEMA IF NOT EXISTS gold;", "")
    engine = create_engine(DB_URL)
    try:
        with engine.begin() as conn:
            conn.execute(text(tables_ddl))
            print(f"  v tables and indexes created")
    finally:
        engine.dispose()
    ...
    try:
        with verify_engine.connect() as conn:
            tables = ['dim_datetime','dim_date','dim_apartment','dim_room','dim_device',
